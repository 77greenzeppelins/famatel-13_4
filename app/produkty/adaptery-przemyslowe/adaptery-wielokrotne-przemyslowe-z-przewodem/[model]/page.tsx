...
export async function generateMetadata({ params }: Props, parent: ResolvingMetadata): Promise<Metadata> {
    ...
    const matchingObject = cat3_allModelsCard_data[2].find((item) => item.modelPathSegmant === model)?.altName;
    ...
    return {
        ...
        alternates: {
            canonical: `${metadataText.cat3.subCat3.canonical}/${model}`
        }
    };
}
export default function AdapteryPrzemysloweWielokrotneZPrzewodemModelPage() {
    /**JSX**/
    return (
        <ModelPageWrapper>
            <ModelPageContent />
