...
export async function generateMetadata({ params }: Props, parent: ResolvingMetadata): Promise<Metadata> {
    ...
    const matchingObject = cat2_allModelsCard_data[2].find((item) => item.modelPathSegmant === model)?.altName;
    ...
    return {
        ...
        keywords: model,
        alternates: {
            canonical: `${metadataText.cat2.subCat3.canonical}/${model}`
        }
    };
}

export default function GniazdaZRozlacznikiemIBlokadaIOpcjamiModelPage() {
    /**JSX**/
    return (
        <ModelPageWrapper>
            <ModelPageContent />
