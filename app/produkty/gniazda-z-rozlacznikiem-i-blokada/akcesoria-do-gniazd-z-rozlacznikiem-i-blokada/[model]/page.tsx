...
export async function generateMetadata({ params }: Props, parent: ResolvingMetadata): Promise<Metadata> {
    ...
    const matchingObject = cat2_allModelsCard_data[3].find((item) => item.modelPathSegmant === model)?.altName;
    ...
    return {
        ...
        keywords: model,
        alternates: {
            canonical: `${metadataText.cat2.subCat4.canonical}/${model}`
        }
    };
}

export default function GniazdaZRozlacznikiemIBlokadaAkcesoriaModelPage() {
    /**JSX**/
    return (
        <ModelPageWrapper>
            <ModelPageContent />
