...
export async function generateMetadata({ params }: Props, parent: ResolvingMetadata): Promise<Metadata> {
    ...
    const matchingObject = cat2_allModelsCard_data[1].find((item) => item.modelPathSegmant === model)?.altName;
    ...
    return {
        ...
        keywords: model,
        alternates: {
            canonical: `${metadataText.cat2.subCat2.canonical}/${model}`
        }
    };
}

export default function GniazdaZRozlacznikiemIBlokadaStaleModelPage() {
    /**JSX**/
    return (
        <ModelPageWrapper>
            <ModelPageContent />
