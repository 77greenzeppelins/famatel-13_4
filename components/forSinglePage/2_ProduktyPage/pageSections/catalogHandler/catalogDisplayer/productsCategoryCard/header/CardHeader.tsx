...
const CardHeader = ({ label, i, setExpanded, isOpen, categoryNumber, href }: Props) => {
    /**JSX**/
    return isOpen ? (
        <h1 className="flex w-full ">
            <div
                onClick={() => setExpanded(isOpen ? false : i)}
                className={`flex w-11/12 items-end h-[80px] x3xx:h-[60px] py-1 cursor-pointer ${isOpen ? 'border-b' : 'border-none'} group hover:border-light ${styles.basicAnimation}  ${
                    isOpen ? 'border-corpo' : 'border-dark'
                } `}
            >
                <p className="flex items-center w-full gap-x-4 text-medium-part">
                    <span className={`w-[50px] group-hover:text-light  ${styles.basicAnimation}  ${isOpen ? 'text-corpo' : 'text-grey'}`}>{categoryNumber}</span>
                    <span className={`w-full group-hover:text-light  ${styles.basicAnimation}  ${isOpen ? 'text-corpo' : 'text-grey'}`}>{label}</span>
                </p>
            </div>
